...
Uso:
  python3 generate_codeql_page.py <sarif_file_or_dir> <output_dir> [--repo REPO] [--branch BRANCH] [--run-id RUN_ID]
"""
    ...
    for run in data.get("runs", []):
        ...
        for result in run.get("results", []):
            ...
            if locs:
                ...
                snippet = region.get("snippet", {}).get("text", "") if isinstance(region.get("snippet"), dict) else ""
...
def finding_card_html(idx: int, f: dict) -> str:
    ...
    return f"""
<div class="finding-card sev-card-{f['level']}" data-level="{f['level']}" data-rule="{f['rule_id']}">
...
def generate_html(
    ...
    status_color = "var(--green)" if passed else ("var(--red)" if counts["critical"] > 0 else "var(--amber)")
...
<div class="empty-state">
  ...
  <p>El análisis CodeQL no encontró vulnerabilidades en el código.</p>
    ...
    rules_sidebar = "".join(
        ...
        f'<span class="method-dot" style="background:var(--text3)"></span>'
        f'{r}</div>'
...
<head>
  ...
  <title>CodeQL Security Report — {repo}</title>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    /* severity counters in sidebar */
    .sev-counter {{
      margin-left: auto; font-size: 11px; font-weight: 700;
      font-family: var(--mono); padding: 1px 7px;
    ...
    .hero-sub {{ font-size: 14px; color: var(--text2); line-height: 1.65; max-width: 520px; margin-bottom: 24px; }}
    ...
    .status-banner {{
      ...
      margin: 0 48px 0; padding: 14px 20px;
...
<body>
  ...
  <header class="header">
    <div class="header-left">
      <div class="logo-mark">CQL</div>
      <span class="header-title">{repo}</span>
      <div class="header-divider" style="width:1px;height:18px;background:var(--border);"></div>
      <span class="header-subtitle" style="font-size:12px;color:var(--text3);">Security Report</span>
    </div>
  ...
  <div class="layout">
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-item" data-level="medium" onclick="filterLevel('medium')">
        <span class="method-dot" style="background:var(--blue)"></span>
        Medio
      ...
      {'<div class="sidebar-item" style="cursor:default;font-size:11px;color:var(--text3);"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Run #' + run_id + '</div>' if run_id else ''}
    </aside>
    ...
    <main class="main">
      ...
      <section class="hero">
        ...
        <p class="hero-sub">Análisis estático de seguridad ejecutado por GitHub CodeQL sobre el repositorio <strong>{repo}</strong>. Identifica vulnerabilidades, patrones inseguros y malas prácticas antes de que lleguen a producción.</p>
      ...
      <div class="status-banner">
        ...
        <div class="status-text">
          ...
          <p>{'No se detectaron vulnerabilidades en el código analizado.' if passed else f"Revisa los hallazgos y aplica las correcciones sugeridas antes de fusionar."}</p>
        </div>
      ...
      <div class="stats">
        <div class="stat-item">
          <span class="stat-value c-accent">{total}</span>
          <span class="stat-label">Total</span>
        </div>
        <div class="stat-item">
          <span class="stat-value c-red">{counts['critical']}</span>
          <span class="stat-label">Crítico</span>
        </div>
        <div class="stat-item">
          <span class="stat-value c-amber">{counts['high']}</span>
          <span class="stat-label">Alto</span>
        </div>
        <div class="stat-item">
          <span class="stat-value c-blue">{counts['medium']}</span>
          <span class="stat-label">Medio</span>
        </div>
        <div class="stat-item">
          <span class="stat-value c-green">{counts['low']}</span>
  ...
  <script>
    ...
    function applyFilters() {{
      ...
      document.querySelectorAll('.finding-card').forEach(function(card) {{
        ...
        const ruleOk = !_activeRule || rule === _activeRule;
        const txtOk  = !q || txt.includes(q);
        card.style.display = (lvlOk && ruleOk && txtOk) ? '' : 'none';
      }});
    }}
    ...
    function filterLevel(level) {{
      ...
      document.querySelectorAll('.rule-filter').forEach(function(el) {{ el.classList.remove('active'); }});
    ...
    function filterRule(ruleId) {{
      ...
      document.querySelectorAll('.rule-filter').forEach(function(el) {{
...
def _donut_section(counts: dict, total: int) -> str:
    """Genera la sección del donut chart SVG."""
    if total == 0:
        return ""
...
def main():
    parser = argparse.ArgumentParser(description="Genera página HTML de reporte CodeQL")
    ...
    parser.add_argument("--repo",     default="mi-repo", help="Nombre del repositorio (owner/name)")
    parser.add_argument("--branch",   default="develop",  help="Rama analizada")
    parser.add_argument("--run-id",   default="",         help="ID del run de GitHub Actions")
    ...
    if not sarif_files:
        ...
        print(f"📄 Archivos SARIF encontrados: {len(sarif_files)}")
        findings = []
