...
/**
 * ====================================================================
 * Módulo: Dashboard Gerencial Service (Pruebas Unitarias)
 * HU: HU16 - Dashboard Gerencial
 *
 * Responsabilidades:
 * - Probar la invocación de múltiples métodos del repositorio en paralelo
 *   (Resumen, Gráficas, Operaciones, Rendimiento, Historial).
 * - Confirmar el formateo correcto de la respuesta agrupada con su modelo.
 * - Comprobar la propagación y rechazo temprano de errores en la base de datos.
 * ====================================================================
 */
...
beforeAll(async () => {
  ({ dashboardGerencialRepository } =
    await import("../../../src/functions/dashboard-gerencial-services/dashboardGerencialRepository.mjs"));
  ({ buildDashboardGerencialResponse } =
    await import("../../../src/functions/dashboard-gerencial-services/dashboardGerencialModel.mjs"));
...
describe("Dashboard Gerencial - Service", () => {
  ...
  it("debería propagar errores si algún repositorio falla", async () => {
    ...
    await expect(getDashboardGerencialService("hoy", "")).rejects.toThrow("Fallo la base de datos");
