...
describe("dashboardRepository", () => {
  ...
  it("getAlertas mapea alertas activas y contratos por expirar", async () => {
    query
      .mockResolvedValueOnce({
        rows: [{ id: "a1", placa: "ABC-123", velocidad_kmh: "120", estado: "EXCESO_VELOCIDAD", fecha_hora: "2026-05-30T10:00:00Z" }],
      })
      .mockResolvedValueOnce({
    ...
    expect(result.alertasActivas).toEqual([
      { id: "a1", placa: "ABC-123", velocidad_kmh: 120, estado: "EXCESO_VELOCIDAD", fecha_hora: "2026-05-30T10:00:00Z" },
    ]);
  ...
  it("getGraficas retorna agrupaciones numericas", async () => {
    query
      ...
      .mockResolvedValueOnce({ rows: [{ estado: "DISPONIBLE", total: "4" }, { estado: "MANTENIMIENTO", total: "4" }] });
  ...
  it("getTopCamiones mapea kilometros por unidad", async () => {
    ...
    expect(result).toEqual([{ unidad: "u1", placa: "ABC-123", modelo: "Volvo", kilometros: 250.5, horas: 5, eficiencia: 50.1 }]);
  });

  it("getContratos mapea contratos activos", async () => {
    query.mockResolvedValue({
      rows: [{ id: "c1", cliente: "Cliente", tarifa: "1500.00", fecha_fin: "2026-06-01", camiones_asignados: "3" }],
    });
